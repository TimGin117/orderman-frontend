...
export default class OrderTable extends Component {
  ...
  mockApi = (len) => {
    return new Promise((resolve) => {
        axios.get('http://localhost:8080/order/showPage',{
          params:{
  ...
  deleteData=(id)=>{
    ...
    axios.post('http://localhost:8080/order/delete/'+id
        ).then(response=>{
          console.log(response);
          if(response.data.code==0){
            this.fetchData();
  ...
  // 获取总记录数
  fetchTotal=()=>{
    axios.get('http://localhost:8080/order/total')
        .then(response=>{
          console.log(response);
          if(response.data.code==0)
  ...
  renderOper = (value, index, record) => {
    return (
      <div>
        <Button type="normal" warning onClick={this.handleDelete.bind(this,record.orderId)}>
          删除
        </Button>
      </div>
    );
      ...
      <div style={styles.container}>
        ...
        <IceContainer>
          <Table loading={isLoading} dataSource={data} hasBorder={false}>
            ...
            <Table.Column title="客户编号" dataIndex="buyerId" />
            <Table.Column title="客户姓名" dataIndex="buyerName" />
            <Table.Column title="创键时间" dataIndex="createTime" />
